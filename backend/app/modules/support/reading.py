...
def visible_state(run, job):
    if run.state == "queued":
        return "processing" if job.state == "running" else job.state
    return run.state
...
def detail(db, workspace_id, actor_id, run_id):
    ...
    return {
        ...
        "handoff": {
            ...
            "handoff_elapsed_ms": (handoff.submitted_at - handoff.created_at).total_seconds() * 1000
            if handoff.submitted_at
            else None,
        }
        if handoff
        else None,
