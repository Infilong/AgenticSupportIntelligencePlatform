...
from app.core.language import SupportedLanguage, select_support_language
...
from app.services.graph_model_errors import _budget_failure_output, _provider_failure_output
from app.services.graph_retrieval import retrieve_evidence
...
from app.services.mock_support_answer import mock_support_answer as _mock_answer
...
from app.services.support_graph_builder import run_support_graph
...
class SupportAgentGraphRunner:
    ...
    def run(self, initial_state: SupportAgentState, *, before_node=None) -> SupportAgentState:
        return run_support_graph(self, initial_state, before_node=before_node)

    def detect_language(self, state: SupportAgentState) -> SupportAgentState:
        started = time.perf_counter()
        output = select_support_language(state["input_message"], state.get("requested_language"))
    ...
    def classify_intent(self, state: SupportAgentState) -> SupportAgentState:
        ...
        prompt_template = AgentPromptService(self.db, state).get_active_or_create_default(
        ...
        try:
            ai_response = run_classification_chain(
                provider=BudgetedModelProvider(self.db),
                workspace_id=UUID(state["workspace_id"]),
                language=language,
    ...
    def retrieve_evidence(self, state: SupportAgentState) -> SupportAgentState:
        return retrieve_evidence(self, state, top_k=_retrieval_top_k(state),
