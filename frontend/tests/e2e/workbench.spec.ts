...
test('operator requests clarification in Japanese and processes customer details in a fresh attempt', async ({ page }) => {
  test.setTimeout(120000);
  await login(page); await create(page, '通常の返金申請の期限はいつですか？', 'ja');
  await expect(page.locator('.run-heading').getByRole('status')).toHaveText('Waiting for development response', { timeout: 90000 });
  await page.getByText('Development response controls', { exact: true }).click();
  const source = page.getByLabel('Supporting passage', { exact: true });
  const option = source.locator('option').filter({ hasText: 'REFUND-STANDARD' }).first();
  ...
  await source.selectOption((await option.getAttribute('value'))!);
  await page.getByLabel('Exact supporting quote').fill(await page.locator('.development-form .source-excerpt').innerText());
  const draft = '年間サブスクリプションの初回購入から14暦日以内です。';
  await page.getByLabel('Development answer', { exact: true }).fill(draft);
  await page.getByLabel('Review category', { exact: true }).selectOption('policy_exception');
  await page.getByRole('button', { name: 'Submit development draft', exact: true }).click();
  await expect(page.locator('.run-heading').getByRole('status')).toHaveText('Needs review', { timeout: 20000 });
  const originalUrl = page.url();
  await page.getByRole('button', { name: 'Sign out', exact: true }).click();
  await login(page, 'operator'); await page.goto(originalUrl);
  await page.getByLabel('Decision', { exact: true }).selectOption('clarify');
  const question = '初回購入ですか、それとも更新ですか？購入日も教えてください。';
  await page.getByLabel('Question for the customer').fill(question);
  await page.getByLabel('Reason for your decision').fill('Purchase type and date are needed before reviewing eligibility.');
  await page.getByRole('button', { name: 'Record clarification request', exact: true }).click();
  await expect(page.getByRole('heading', { name: 'Clarification requested', exact: true })).toBeVisible({ timeout: 20000 });
  await expect(page.locator('.run-result > .response-text')).toHaveText(question);
  await expect(page.locator('.run-result > .response-text')).toHaveAttribute('lang', 'ja');
  await expect(page.getByRole('heading', { name: 'Approved response', exact: true })).toHaveCount(0);
  await page.getByText('Original unapproved draft', { exact: true }).click();
  await expect(page.locator('.run-result details .response-text')).toHaveText(draft);
  await page.setViewportSize({ width: 360, height: 900 });
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  await page.screenshot({ path: test.info().outputPath('clarification-request-ja-360.png'), fullPage: true });
  await page.getByText('Add customer details', { exact: true }).click();
  const reply = '初回の年間サブスクリプション購入で、昨日購入しました。';
  await page.getByLabel('Additional customer details').fill(reply);
  await page.getByRole('button', { name: 'Process with added details', exact: true }).click();
  await expect(page).not.toHaveURL(originalUrl);
  await expect(page.locator('.run-heading').getByRole('status')).toHaveText('Waiting for development response', { timeout: 90000 });
  await expect(page.getByText('Review decision · clarify', { exact: true })).toHaveCount(0);
  await page.getByText('Processing input · attempt 2', { exact: true }).click();
