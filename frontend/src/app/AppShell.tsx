...
const tabs: TabDefinition[] = [
  ...
  { id: "documents", label: "Knowledge", token: "◆", group: "Build", purpose: "Manage RAG policies, FAQs, versions, chunks, and citations.", requiredPermissions: ["knowledge:read"] },
  { id: "agent", label: "Agents", token: "⚙", group: "Build", purpose: "Configure and run governed LangGraph agent workflows.", requiredPermissions: ["agents:read"] },
  { id: "tools", label: "Tools", token: "⌘", group: "Build", purpose: "Inspect agent tools, schemas, permissions, usage, and errors.", requiredPermissions: ["tools:read"] },
  { id: "guardrails", label: "Guardrails", token: "◇", group: "Build", purpose: "Inspect governance checks, policy coverage, failures, and review routing.", requiredPermissions: ["guardrails:read"] },
  { id: "trace", label: "Runs & traces", token: "↗", group: "Operate", purpose: "Inspect graph state, tools, guardrails, evidence, and model calls.", requiredPermissions: ["traces:read"] },
  { id: "reviews", label: "Human review", token: "!", group: "Operate", purpose: "Resolve blocked, risky, low-confidence, or unsupported runs.", requiredPermissions: ["reviews:read"] },
  { id: "evaluations", label: "Evaluations", token: "◎", group: "Evaluate", purpose: "Compare quality, routing, language preservation, and baselines.", requiredPermissions: ["evaluations:read"] },
  { id: "costs", label: "Usage & costs", token: "¢", group: "Evaluate", purpose: "Monitor tokens, latency, cache behavior, model purpose, and spend.", requiredPermissions: ["costs:read"] },
  { id: "members", label: "Members", token: "@", group: "Admin", purpose: "Manage workspace members, owner rights, and available permissions.", requiredPermissions: ["members:read"] },
  { id: "prompts", label: "Prompts", token: "¶", group: "Admin", purpose: "Version and activate LangChain prompt templates by language.", requiredPermissions: ["prompts:read"] },
  { id: "models", label: "Models", token: "◉", group: "Admin", purpose: "Control provider, model purpose, context, and token pricing.", requiredPermissions: ["models:read"] },
  { id: "system", label: "System health", token: "+", group: "Admin", purpose: "Inspect provider readiness, limits, failures, data, and governance posture.", requiredPermissions: ["system:read"] },
  { id: "audit", label: "Audit", token: "§", group: "Admin", purpose: "Inspect accountable workspace and AI operations changes.", requiredPermissions: ["audit:read"] },
  { id: "settings", label: "Settings", token: "∴", group: "Settings", purpose: "Manage workspace identity and route to provider, permission, budget, and health settings.", requiredPermissions: ["settings:read"] },
];
...
const evaluationCasesPlaceholder = "Paste JSONL evaluation cases here. Include id, language, input message, expected route, expected evidence, and token limits.";

const promptTemplatePlaceholder = "Write the prompt template source here. Include explicit variables such as language, input message, and evidence.";
  ...
  if (!token) {
    return (
      <main className="auth-page">
        ...
        <form className="auth-panel" onSubmit={handleAuth} autoComplete="off">
          ...
          {authMode === "register" && (
            <label>
              Display name
              <input value={displayName} onChange={(event) => setDisplayName(event.target.value)} placeholder="Your name" autoComplete="off" />
            </label>
          )}
          <label>
            Email
            <input value={email} onChange={(event) => setEmail(event.target.value)} placeholder="Email address" autoComplete="off" />
          </label>
          <label>
            Password
      ...
      <div className="evaluation-console">
        ...
        <section className="evaluation-workbench">
          ...
          <form className="panel stack evaluation-run-panel" onSubmit={runEvaluation}>
            ...
            <label>
              JSONL cases
              <textarea rows={16} value={evaluationCases} onChange={(event) => setEvaluationCases(event.target.value)} placeholder={evaluationCasesPlaceholder} />
            </label>
            <div className="run-action-bar">
      ...
      <div className="settings-console prompt-console">
        ...
        <section className="settings-workbench">
          <form className="panel stack settings-editor-panel" onSubmit={createPromptTemplateVersion}>
            ...
            <label>
              Template source
              <textarea rows={16} value={promptText} disabled={!canManagePrompts || loading} onChange={(event) => setPromptText(event.target.value)} placeholder={promptTemplatePlaceholder} />
            </label>
            <div className="run-action-bar">
