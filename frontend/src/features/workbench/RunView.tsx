...
export function RunView({ workspace, runId, onChanged }: { workspace: Workspace; runId: string; onChanged: () => void }) {
  ...
  return <div className="run-view"><Link className="back-to-messages" to={`/w/${workspace.id}`}>← All messages</Link>
    ...
    {run && <><div className="run-heading"><span className={`run-state ${run.state}`} role="status">{stateLabel(run.state)}</span><span className="muted">{languageLabel(run.language)}</span></div>
      ...
      <section className="run-result"><h2>{run.state === 'draft' ? 'Response draft' : 'Next step'}</h2>
        {run.draft ? <><p className="response-text" lang={run.language}>{run.draft}</p>{run.state === 'draft' && <p className="muted">Development draft · Not approved for sending</p>}</> : <p className="muted">{run.state === 'waiting_development' ? 'Retrieval is complete. An administrator can supply a development response using the supporting evidence.' : run.state === 'cancelled' ? 'This processing attempt was cancelled. The original message and processing history are preserved.' : run.state === 'failed' ? 'Processing stopped before a response was ready. Inspect the details below. You can start a new message while retry support is being completed.' : 'Your message is saved. Processing will continue in the background.'}</p>}
      ...
      {workspace.role === 'admin' && run.state === 'waiting_development' && <details className="development-controls" onToggle={event => setDevelopmentOpen(event.currentTarget.open)}><summary>Development response controls</summary>{developmentOpen && <DevelopmentResponse path={path} onSubmitted={changed} />}</details>}
