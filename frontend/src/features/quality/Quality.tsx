...
const roles: Record<string, string> = { generate: 'Answer generation', embed_query: 'Question understanding', embed_passage: 'Knowledge indexing', rerank: 'Evidence ranking' };
...
export function Quality({ workspace }: { workspace: Workspace }) {
  ...
  return <><div className="page-heading"><p className="eyebrow">WORKSPACE ACTIVITY</p><h1>Quality</h1><p className="muted">Inspect model activity, saved retrieval checks and measurement gaps.</p></div>
    ...
    {totals && <><div className="usage-cards"><section className="inbox-panel section-padding"><h3>Recorded calls</h3><strong>{number(totals.calls)}</strong><p>{number(totals.succeeded)} succeeded · {number(totals.failed)} failed</p><p className="muted">{number(totals.started)} started · {number(totals.uncertain)} uncertain</p></section>
      ...
      <section className="inbox-panel section-padding"><h3>Generated output tokens</h3><strong>{number(totals.output_tokens)}</strong><p className="muted">{number(totals.missing_output_tokens)} generation calls without output measurements</p></section>
      ...
      <p className="usage-note">Each row combines calls to the same model for the same role during this period. These are recorded operations, not an answer-quality score. Local models have no external API charge; they still use computer resources. Input tokens measure supplied text; output tokens measure generated text. Missing measurements are excluded from sums; an uncertain call may still have used resources. Development handoffs are not automatic model calls.</p>
      <section className="inbox-panel"><div className="panel-heading"><h2>Models and their roles</h2></div>{!totals.calls ? <div className="section-padding"><h3>No recorded model calls</h3><p>Index a knowledge document or process a message to see activity here.</p></div> : <div className="usage-table-scroll" role="region" aria-label="Model usage results" tabIndex={0}><table className="usage-table"><thead><tr><th>Model / operation</th><th>Calls</th><th>Failed / uncertain</th><th>Input tokens</th><th>Output tokens</th><th>Total call time</th><th>External charge</th></tr></thead><tbody>{report.data!.models.map(model => <tr key={`${model.provider}/${model.model}/${model.operation}`}><td><strong>{model.model}</strong><p>{roles[model.operation] ?? model.operation} · {model.provider.replaceAll('_', ' ')}</p></td><td>{number(model.calls)}</td><td>{model.failed} / {model.uncertain}</td><td>{number(model.input_tokens)}<small>{model.missing_tokens} unmeasured</small></td><td>{model.operation === 'generate' ? <>{number(model.output_tokens)}<small>{model.missing_output_tokens} unmeasured</small></> : 'Not applicable'}</td><td>{(model.recorded_duration_ms / 1000).toFixed(2)} s<small>{model.missing_duration} unmeasured</small></td><td>${model.recorded_cost_usd.toFixed(4)}<small>{model.missing_cost} unmeasured</small></td></tr>)}</tbody></table></div>}</section>
      <p className="muted usage-note">{report.data!.more_models ? 'Showing the 20 most-used model/operation groups. Totals include every group. ' : ''}Model names show historical usage, not which models are currently loaded. Durations sum recorded call time; they are not workflow wall time. Window ends {new Date(report.data!.until).toLocaleString()}.</p>
    </>}
    </>}
  </>;
}
