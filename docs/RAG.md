...
claim that BM25, automated grounding validation or external generation already work.
...
evidence handling and trace completeness. **Missing:** independent BM25/fusion, full model-call
...
| 7. Lexical retrieval | EN identifier terms and CJK bigrams are stored, not searched | Implement actual BM25 over term frequencies/document lengths. A set intersection, `text.split()` or PostgreSQL `ts_rank` must not be relabelled BM25. |
...
- **P2 resolved here:** demonstrated scorer false positives/result bounds. Remaining: BM25/fusion,
