...
| 11. Generation | Manual graphs pause for attributed contributions; configured ordinary local graphs now call LangChain/Ollama and record usage and routing. [Current topology](ARCHITECTURE.md) | No paid/cloud connectivity or full semantic-quality proof. Local input and output usage are in the ledger and usage report, with original usage retained in the stored response; development wait is not inference latency. |
